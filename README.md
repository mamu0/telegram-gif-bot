...
Runs on **Render** with **webhooks** - only processes requests when users interact with it. No polling, no continuous server running.
...
- 🌐 **Serverless**: Runs on Render (free tier), only processes when needed
...
- Render account (free from [render.com](https://render.com))
...
5. Add **Environment Variables**:
   ...
   - `WEBHOOK_URL` = Your Render app URL (e.g., `https://telegram-gif-bot-xxx.onrender.com`)
   - `RESULTS_LIMIT` = `10`
6. Click **Deploy**
...
- `<YOUR_WEBHOOK_URL>` with your Render URL (e.g., `https://telegram-gif-bot-xxx.onrender.com`)
...
- **DO NOT** commit your `.env` file - use environment variables on Render
...
3. **Render Wakes Up**: Processes the request (fast cold start)
4. **Bot Responds**: Returns matching GIFs
5. **Render Sleeps**: Goes dormant until next request
